...
class LM_LSTM_CRF(nn.Module):
    ...
    def __init__(self, charset_size, char_emb_dim, char_rnn_dim, char_rnn_layers,
        ...
        self.subword_hw = Highway(2 * self.char_rnn_dim, num_layers=self.highway_layers,
                                  dropout=dropout).to('cuda')  # highway to transform combined forward and backward char LSTM outputs for use in the word BLSTM
        ...
        self.fc1 = nn.Linear((self.word_rnn_dim*2)+2, self.hidden_size) #Note that the extra +1 is for the pos tag concat
    ...
    def forward(self, words, cmaps_f, cmaps_b, cmarkers_f, cmarkers_b, wmaps, wmap_lengths, cmap_lengths, probs, tmaps,puncts):
        ...
        puncts = (puncts.unsqueeze_(-1)).expand(list(att_output.size())[0],list(att_output.size())[1],1).float()
        att_output = torch.cat([att_output, tmaps,puncts],2)
